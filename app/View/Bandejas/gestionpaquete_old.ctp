...
<div class="paquetes form" id="divConfigPaquete" text-align: center>   
    ...
    <br/>
        <fieldset>
            <h4><?php echo __('Detalle de la Solicitud'); ?></h4>
    ...
    <button type="button" name="butVerTraza" id="butVerTraza" class="btn btn-info" onclick="mostrarTrazabilidad(<?php echo $paqueteId; ?>,'<?php echo __($estado); ?>','div_trazabilidad');">Trazabilidad</button><br> 
    
    <br/>
    <br/>
    <fieldset>
        <legend><h4>Documentos Adjuntos</h4></legend>
    ...
    <table width="100%">
        <tr>
            ...
            <textarea name="observacion" id="observacion" cols="60" rows="15" class="textAreaLoc" onblur="guardarObserv('<?php echo $datosUsuarioLogin['nombre']?>');"></textarea>
            </td>
            <?php } ?>
            <td width="50%">
            <legend><h4>Observaciones:</h4></legend>
            <textarea name="obsGeneral" cols="60" rows="15" class="textAreaLoc" readonly id="obsGeneral"><?php if (isset($observacion['0'])){echo $observacion['0']['Observacione']['descripcion']; } ?></textarea>            
            </td>
        </tr>
    </table>    
    <?php
