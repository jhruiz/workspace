 ...
 */
	public function delete($id = null) {
        ...
        public function addSecuencia(){
            ...
            $estadoSiguiente = $arrBandEst['0']['Relacionbandejasestado']['estado_id'];
            ...
            if($this->Bandejasestado->relacionSecuenciaExiste($bandejaInicial, $estadoSiguiente)){
                ...
                if ($this->Bandejasestado->guardarBandejasEstados($bandejaInicial,$estadoSiguiente,$etiquetaCambio,$analisisCargas)) {
    ...
    public function gestionpaquete(){
        if($this->request->is('post')){
            ...
            $varJVM = 'jvm';
            $dirJVM = $this->Configuraciondato->obtenerInfo($varJVM);
            
            $varAgente = 'Agente_ConsolidaPDF';
            ...
            /*if(count($documentosPaq) > 1){
                ...
                //Se genera el archivo plano con los datos de los archivos a concatenar
                $infoConcatPDF = $this->generarPlanoConcatenar($arrDocumentos,$dirTemp,$paqueteId,$numeroCredencial);
    ...
    public function guardargestionpaquete(){
        if($this->request->is('post')){
            ...
            if(isset($arrSecuencia['Bandejasestado']) && $arrSecuencia['Bandejasestado']['analisiscargas'] == '1' && $paqUsrId == ""){
                ...
                $paqUsrId = $this->asignacionAnalisisCargas($paqueteId, $nuevoEstadoId, $arrOficio['Paquete']['oficina_id']);                
...
//                    $this->correoDevolucionPaquete($arrOficio, $motivoRechazoId, $obsDevOficio, $arrBandejaDestino, $paqUsrId);
    ...
    public function generarPlanoConcatenar($arrDocumentos,$dirTemp,$paqueteId,$numeroCredencial){
        //Se obtiene la información del usuario que realiza la gestion
        $usuarioId = $this->Auth->user('id');
        
        //Se obtiene la fecha en la forma Ymd_His para asignarla al nombre
        $fechaAct = date('Ymd_His');
                
        //Se obtiene la ruta donde se almacenara el pdf generado y su nombre
        $pathEndPdf = dirname($arrDocumentos['0']) . '/' . $usuarioId . '_' . $numeroCredencial . '_' . $fechaAct . '.pdf';

        $nombre_archivo = $dirTemp . $paqueteId . "_" . $fechaAct . ".txt"; //variable con el nombre del archivo que vamos a crear
        
        $infoConcatPDF['rutaTxt'] = $nombre_archivo;
        $infoConcatPDF['pathPdf'] = $numeroCredencial . '/' . $usuarioId . '_' . $numeroCredencial . '_' . $fechaAct . '.pdf';
        
        $saltoLinea = "\r\n";
        if($archivo = fopen($nombre_archivo, "a"))
        {
            if(fwrite($archivo, 'RutaDestino:' . $pathEndPdf . $saltoLinea)){
                foreach ($arrDocumentos as $docs){
                    fwrite($archivo, 'PDF:' . $docs . $saltoLinea);
}
            }
            fclose($archivo);
        }
        
        return $infoConcatPDF;
    }
