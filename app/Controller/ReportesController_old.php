...
//        }       
    ...
    public function generar_tiempospromedio() {
        ...
        $tipo_reporte = 0;
        if(isset($this->request->data['Reporte']['credencial'])){
        ...
        $trazas = array();
        if ($tipo_reporte == "1") {
            $trazas = $this->Trazabilidade->find('all', array('recursive' => 0));

            if (!empty($credencial)) {
                $trazas = $this->Trazabilidade->find('all', array('conditions' => array('Paquete.numerocredencial' => $credencial), 'recursive' => 0));
            }
        } else {
            ...
            if (!empty($credencial)) {
                $this->Paginator->settings['conditions'] = array('Paquete.numerocredencial' => $credencial);
            }
        ...
        for ($i = 0; $i < count($trazas); $i++) {
            $usuarioPost = $this->Usuario->find('first', array('conditions' => array('Usuario.id' => $trazas[$i]['Paquetesusuario']['usuario_id']), 'recursive' => -1));

            if (isset($usuarioPost['Usuario']['nombre'])) {
                $trazas[$i]['Usuario']['nombre'] = $usuarioPost['Usuario']['nombre'];
            }

            $arrOficinas = $this->Oficina->find('first', array('conditions' => array('Oficina.id' => $trazas[$i]['Paquete']['oficina_id']), 'recursive' => -1));
            if (isset($trazas[$i]['Paquete']['oficina_id'])) {
                $trazas[$i]['Paquete']['nombre_oficina'] = $arrOficinas['Oficina']['descripcion'];
            } else {
                continue;
            }
        }        

        $texto_tit = "Reporte - Tiempos promedio por estado" . (empty($credencial) ? "" : " Credencial {$credencial}");
